...
class PhotoTableViewCell: UITableViewCell {
    ...
    func setUpUI()  {
        ...
        let headerTapGesture = UITapGestureRecognizer(target: self, action: #selector(self.headerViewTapped))
...
//        copyRightLabel.text = "来自于Tumblr"
//        copyRightLabel.font = UIFont.systemFont(ofSize: 14)
//        copyRightLabel.textColor = UIColor.darkGray
...
//        photoImageView.addGestureRecognizer(tapGesture)
    ...
    @objc func headerViewTapped() {
        delegate?.showBlogView(row: rowIndex)
    }
...
//    }
    ...
    func config(data: TagSearchResponse, row: Int) {
        ...
        blogIdLabel.text = data.blog_name
...
//        if let photoInfo = PhotoTableViewCell.choosePhoto(data: data, width: 400) {
...
//
        ...
        let rowHeight = 60 + 44 + summaryHeight + photoViewHeight + cellSeperatorHeight + summaryLabelTopAndBottomPadding
...
extension PhotoTableViewCell: PhotoContainerViewDelegate {
    func imageTapped(index: Int) {
        delegate?.photoTapped(row: rowIndex, index: index)
    }
}

extension PhotoTableViewCell: TTGTextTagCollectionViewDelegate {
